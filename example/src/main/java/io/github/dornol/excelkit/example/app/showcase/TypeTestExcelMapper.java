...
public class TypeTestExcelMapper {
    ...
    public static ExcelHandler getHandler(Stream<TypeTestDto> stream) {
        ...
        return ExcelWriter.<TypeTestDto>create().headerColor(ExcelColor.of(147, 252, 42)).maxRows(1_000_000)
                ...
                .column("integer", TypeTestDto::anInteger, cfg -> cfg.type(ExcelDataType.INTEGER))
                .column("localdatetime", TypeTestDto::aLocalDateTime, cfg -> cfg.type(ExcelDataType.DATETIME))
                .column("localdate", TypeTestDto::aLocalDate, cfg -> cfg.type(ExcelDataType.DATE))
                .column("localtime", TypeTestDto::aLocalTime, cfg -> cfg.type(ExcelDataType.TIME))
                .column("double", TypeTestDto::aDouble, cfg -> cfg.type(ExcelDataType.DOUBLE))
                .column("double_percent", TypeTestDto::aDouble, cfg -> cfg.type(ExcelDataType.DOUBLE_PERCENT))
                .column("float", TypeTestDto::aFloat, cfg -> cfg.type(ExcelDataType.FLOAT))
                .column("float_percent", TypeTestDto::aFloat, cfg -> cfg.type(ExcelDataType.FLOAT_PERCENT))
                .column("boolean", TypeTestDto::aBoolean, cfg -> cfg.type(ExcelDataType.BOOLEAN_TO_YN))
                .column("bigdecimal_long", TypeTestDto::aLongBigDecimal, cfg -> cfg.type(ExcelDataType.BIG_DECIMAL_TO_LONG))
                .column("bigdecimal_double", TypeTestDto::aDoubleBigDecimal, cfg -> cfg.type(ExcelDataType.BIG_DECIMAL_TO_DOUBLE))
                .column("currency", TypeTestDto::aDoubleBigDecimal, cfg -> cfg.type(ExcelDataType.BIG_DECIMAL_TO_DOUBLE).format(ExcelDataFormat.CURRENCY_USD.getFormat()))
                .constColumn("const", random.nextLong(), cfg -> cfg.type(ExcelDataType.LONG))
